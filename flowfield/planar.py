...
class Planar_extensional_flow(FlowfieldBase):

    def __init__(self, **kwargs):
        self.set_property('strain_rate', kwargs['strain_rate'])
        self.set_property('plane', kwargs['plane'])
    ...
    def set_property(self, property_name, property_value):
        ...
        elif  property_name == 'plane':
            self._exp_axis = {'x':0, 'y':1, 'z':2}[property_value[0]]
            self._comp_axis = {'x':0, 'y':1, 'z':2}[property_value[1]]
        else:
    ...
    def get_velocity(self, points, out=None):
        ...
        out[self._comp_axis::3] = (-self._strain_rate
                                    *np.ravel(points)[self._comp_axis::3])
        return out
    ...
    def get_strainrate_tensor(self, points=None, out=None):
        ...
        strainrate_tensor[self._exp_axis,self._exp_axis] = self._strain_rate
        strainrate_tensor[self._comp_axis,self._comp_axis] = -self._strain_rate
        if points is None:
